function [updated_weights] = UpdateWeights_eq10_energyVersion(cellBoundaries,density, neighborsCount, neighborsSharedBoundary, currentRobotsEnergy, wts,Px,Py,Mass, k, alpha)
    ...
    for i = 1:numel(neighborsSharedBoundary)  % get boundary edges that connect the ith robot to its neighbors
        ...
        if(currentSharedBoundary(1)== r || currentSharedBoundary(2) == r)
            ...
            if(currentSharedBoundary(1) == r)
                     weight_diff = (wts(r)-currentRobotsEnergy(r)) - (wts(currentSharedBoundary(2))-currentRobotsEnergy(currentSharedBoundary(2)));       % weight diff = wts(i)*h(i) - wts(j)*h(j) 
            else
                     weight_diff = (wts(r)-currentRobotsEnergy(r)) - (wts(currentSharedBoundary(1))-currentRobotsEnergy(currentSharedBoundary(1)));
            end
